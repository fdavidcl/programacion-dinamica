...
\section{Compraventa de acciones}
	...
	\subsection{Análisis de eficiencia}
		El análisis de este algoritmo es muy sencillo. Tenemos un único bucle de tipo \textit{for} realizando $n-1$ iteraciones, con operaciones $\mathcal{O}(1)$ en el interior. Por tanto, su eficiencia será $\mathcal{O}(n-1) = \mathcal{O}(n)$, es decir, es un algoritmo lineal.
